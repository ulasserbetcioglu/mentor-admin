...
import { 
  ...
  Users, Map, Upload, Image as ImageIcon, ClipboardList, Settings, Beaker, Loader2 // Loader2 eklendi
} from 'lucide-react';
...
export default function MentorModule() {
  ...
  const [formData12, setFormData12] = useState({
    ticariUnvan: '', // Başlangıçta boş bıraktık, DB'den dolacak
    faaliyetKonusu: '',
    vergiDairesi: '',
    vergiNo: '',
    mersisNo: '',
  ...
  const [branches, setBranches] = useState<any[]>([]); // Tip hatası vermemesi için any[] veya interface eklenebilir
  ...
  async function fetchCustomerData() {
    setLoading(true);
    try {
      // Müşteriyi ve bağlı şubelerini çek (limit(1) ile şimdilik ilk müşteriyi alıyoruz)
      ...
      if (data) {
        // DB verisini State formatına eşle
        setFormData12({
          ticariUnvan: data.ticari_unvan || '',
        ...
        if (data.branches && data.branches.length > 0) {
          const mappedBranches = data.branches.map((branch: any) => ({
            ...
            yetkili: branch.yetkili || '',
            metrekare: branch.metrekare || '',
            adres: branch.adres || '',
  ...
  const [stations, setStations] = useState<any[]>([]); // {id, no, location, type}
  ...
  // --- HANDLERS ---
  const handleChange12 = (e: any) => { const { name, value } = e.target; setFormData12(prev => ({ ...prev, [name]: value })); };
  const handleSettings12 = (e: any) => { const { name, value } = e.target; setSettings12(prev => ({ ...prev, [name]: value })); };
  const addBranch = () => { setBranches([...branches, { id: Date.now(), subeAdi: 'Yeni Şube', yetkili: '', metrekare: '', adres: '', telefon: '' }]); };
  const updateBranch = (id: any, field: any, value: any) => { setBranches(branches.map(b => b.id === id ? { ...b, [field]: value } : b)); };
  const removeBranch = (id: any) => { setBranches(branches.filter(b => b.id !== id)); };
  const handleSettings13 = (e: any) => { const { name, value } = e.target; setSettings13(prev => ({ ...prev, [name]: value })); };
  const handleContractChange = (e: any) => { const { name, value } = e.target; setContractData(prev => ({ ...prev, [name]: value })); };
  const handleKapsamChange = (key: any) => { setContractData(prev => ({ ...prev, kapsam: { ...prev.kapsam, [key]: !(prev.kapsam as any)[key] } })); };
  const handleSettings21 = (e: any) => { const { name, value } = e.target; setSettings21(prev => ({ ...prev, [name]: value })); };
  const addPermit = () => { setPermits([...permits, { id: Date.now(), belgeAdi: 'Yeni Belge', belgeNo: '', verilisTarihi: '', gecerlilikTarihi: '', verenKurum: '' }]); };
  const updatePermit = (id: any, field: any, value: any) => { setPermits(permits.map(p => p.id === id ? { ...p, [field]: value } : p)); };
  const removePermit = (id: any) => { setPermits(permits.filter(p => p.id !== id)); };
  const handleSettings31 = (e: any) => { const { name, value } = e.target; setSettings31(prev => ({ ...prev, [name]: value })); };
  const addStaff = () => { setStaff([...staff, { id: Date.now(), adSoyad: 'Yeni Personel', gorev: 'Operatör', sertifikaNo: '', gecerlilikTarihi: '' }]); };
  const updateStaff = (id: any, field: any, value: any) => { setStaff(staff.map(s => s.id === id ? { ...s, [field]: value } : s)); };
  const removeStaff = (id: any) => { setStaff(staff.filter(s => s.id !== id)); };
  const handleSettings32 = (e: any) => { const { name, value } = e.target; setSettings32(prev => ({ ...prev, [name]: value })); };
  const handleImageUpload = (e: any) => { const file = e.target.files[0]; if (file) { const imageUrl = URL.createObjectURL(file); setKrokiImage(imageUrl); } };
  const removeKrokiImage = () => { setKrokiImage(null); if(fileInputRef.current) fileInputRef.current.value = ""; };
  const handleSettings41 = (e: any) => { const { name, value } = e.target; setSettings41(prev => ({ ...prev, [name]: value })); };
  const updateLegend = (id: any, field: any, value: any) => { setLegendItems(legendItems.map(item => item.id === id ? { ...item, [field]: value } : item)); };
  const generateStations = () => { const newStations = []; const start = parseInt(generator.start as any); const end = parseInt(generator.end as any); if (isNaN(start) || isNaN(end) || start > end) return; for (let i = start; i <= end; i++) { const numStr = i < 10 ? `0${i}` : `${i}`; newStations.push({ id: Date.now() + i, no: `${generator.prefix}-${numStr}`, location: '', type: generator.type }); } setStations([...stations, ...newStations]); };
  const updateStation = (id: any, field: any, value: any) => { setStations(stations.map(s => s.id === id ? { ...s, [field]: value } : s)); };
  const removeStation = (id: any) => { setStations(stations.filter(s => s.id !== id)); };
  const clearStations = () => { if(window.confirm('Tüm listeyi silmek istediğinize emin misiniz?')) { setStations([]); } };
  const handleSettings42 = (e: any) => { const { name, value } = e.target; setSettings42(prev => ({ ...prev, [name]: value })); };
  ...
  const updateProduct = (id: any, field: any, value: any) => {
  ...
  // --- ORTAK BİLEŞENLER ---
  const A4Header = ({ title, settings }: any) => (
  ...
  const renderHome = () => (
    <div className="flex flex-col items-center justify-center h-full p-10 bg-gray-50 text-gray-800 overflow-y-auto">
      <div className="flex flex-col items-center mb-10">
        ...
        {loading && <div className="mt-4 flex items-center text-green-600 gap-2"><Loader2 className="animate-spin" /> Veriler Yükleniyor...</div>}
      </div>
  ...
  // ... (Geri kalan render metodları aynı şekilde devam edecek)
  // Editör ve Preview bileşenlerinin kodları çok uzun olduğu için burada tekrar etmedim,
  // ancak App.tsx dosyasının geri kalan kısmını orijinal dosyadaki gibi koruman gerekiyor.
  // Yalnızca yukarıdaki useEffect ve useState güncellemeleri kritik.
  
  // Özetle: renderEditor12, renderEditor13 ... renderA4_52 fonksiyonları aynen kalmalı.
